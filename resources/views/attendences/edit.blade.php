...
@section('content')
    <div class="bg-white shadow-md rounded-lg p-6">
        ...
        @endif
        <form action="{{ route('attendences.update', $attendance->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="date" id="tanggal" name="tanggal" value="{{ old('tanggal', $attendance->tanggal) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700" required>
            </div>

            <div class="mb-4">
                <label for="waktu_masuk" class="block text-gray-700 font-bold mb-2">Waktu Masuk:</label>
                <input type="time" id="waktu_masuk" name="waktu_masuk" value="{{ old('waktu_masuk', $attendance->waktu_masuk) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700">
            </div>

            <div class="mb-4">
                <label for="waktu_keluar" class="block text-gray-700 font-bold mb-2">Waktu Keluar:</label>
                <input type="time" id="waktu_keluar" name="waktu_keluar" value="{{ old('waktu_keluar', $attendance->waktu_keluar) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700">
            </div>
